...
function DodecahedronMesh() {
  ...
  const edgeColor = isLight ? "#1a1a1a" : "#606060";
  ...
  return (
    <group ref={meshRef}>
      ...
      <mesh position={[0, 0, 0.3]} scale={[1, 0.1, 1]} rotation={[Math.PI * 0.5, 0, Math.PI]}>
      ...
      <mesh position={[0, 0, -0.3]} scale={[1, 0.15, 1]} rotation={[Math.PI * 1.5, 0, Math.PI]}>
        <sphereGeometry args={[1, 16, 8, 0, Math.PI * 2, 0, Math.PI * 0.5]} />
        <meshBasicMaterial transparent opacity={0} />
        <Edges threshold={1} color={edgeColor} />
      </mesh>
      ...
      <mesh position={[0, 0, -4]} rotation={[0.3, 0, 0]}>
        ...
        <meshBasicMaterial color={gridColor} wireframe transparent opacity={isLight ? 0.3 : 0.2} />
      </mesh>
    </group>
  );
}
