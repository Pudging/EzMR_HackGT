...
export function ModernDicomViewer({
  ...
  useEffect(() => {
    if (!isOpen || !dicomData?.images?.length) {
      console.log("Modern DICOM viewer not opening:", {
        isOpen,
        hasImages: !!dicomData?.images?.length,
      });
      return;
    }
    ...
    const loadDicomImages = async () => {
      ...
      try {
        ...
        for (let i = 0; i < dicomData.images.length; i++) {
          ...
          try {
            console.log(
              `📥 Loading DICOM ${i + 1}/${dicomData.images.length}: ${imageFile.name}`,
            );
            ...
            for (let j = 0; j < Math.min(pixelData.length, 10000); j++) {
            ...
            if (i === 0) {
              ...
              setMetadata({
                ...
                windowCenter: dicomImage.windowCenter ?? 0,
                windowWidth: dicomImage.windowWidth ?? 0,
              });
            }
          } catch (imageError) {
        ...
        setError(
          `Failed to load DICOM images: ${err instanceof Error ? err.message : "Unknown error"}`,
        );
      } finally {
  ...
  return (
    <div className="bg-opacity-90 fixed inset-0 z-50 flex items-center justify-center bg-black">
      <div className="m-4 flex h-full max-h-screen w-full max-w-7xl flex-col rounded-lg bg-white shadow-xl dark:bg-gray-900">
        {/* Header */}
        ...
        <div className="flex flex-1">
          ...
          <div className="relative flex-1 bg-black">
            {isLoading && (
              <div className="absolute inset-0 flex items-center justify-center text-white">
                <div className="mx-auto max-w-md px-4 text-center">
                  ...
                  <p className="mb-4">
                    Loading {totalSlices} DICOM image
                    {totalSlices !== 1 ? "s" : ""}...
                  </p>
                  ...
                  {loadingProgress.total > 0 && (
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm text-gray-300">
                        <span>
                          Progress: {loadingProgress.current} of{" "}
                  ...
                  <p className="mt-4 text-sm text-gray-300">
                    Modern parser - no Cornerstone needed
                  </p>
                </div>
              </div>
            )}
            ...
            {error && (
              <div className="absolute inset-0 flex items-center justify-center text-white">
                <div className="bg-opacity-75 rounded-lg bg-red-900 p-6 text-center">
            ...
            {!isLoading && totalSlices > 1 && (
              <div className="bg-opacity-70 absolute top-4 left-4 rounded bg-black px-3 py-1 text-sm text-white">
          ...
          {/* Controls Panel */}
          <div className="w-80 space-y-6 overflow-y-auto border-l border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800">
            {/* Playback Controls */}
            {totalSlices > 1 && (
              <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="mt-2 flex justify-center space-x-2">
                      <Button
                        ...
                        className={`px-2 py-1 text-xs ${playSpeed === 2 ? "border-blue-300 bg-blue-100" : ""}`}
                      >
                        2x
                      </Button>
                      <Button
                        ...
                        className={`px-2 py-1 text-xs ${playSpeed === 5 ? "border-blue-300 bg-blue-100" : ""}`}
                      >
                        5x
                      </Button>
                      <Button
                        ...
                        className={`px-2 py-1 text-xs ${playSpeed === 10 ? "border-blue-300 bg-blue-100" : ""}`}
                      >
                        10x
                      </Button>
                      <Button
                        ...
                        className={`px-2 py-1 text-xs ${playSpeed === 15 ? "border-blue-300 bg-blue-100" : ""}`}
                      >
                        15x
                      </Button>
                    </div>
            ...
            {metadata && (
              <div>
                ...
                <div className="space-y-1 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  <p>
                    Pixel Range: {Math.round(metadata.minPixelValue)} -{" "}
                    {Math.round(metadata.maxPixelValue)}
                  </p>
                  <p>
                    Images: {loadedImages.length} / {totalSlices}
                  </p>
                </div>
              </div>
            )}
