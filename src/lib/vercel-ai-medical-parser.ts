...
// Define Zod schema for categorization
...
export type ParsedMedicalData = z.infer<typeof medicalDataSchema>;
export type CategoryExtraction = z.infer<typeof categorizationSchema>;

export class VercelAIMedicalParser {
  private model;
...
Extract all relevant medical information from the notes and structure it according to the schema.
      ...
      console.error('Error parsing medical notes with Vercel AI:', error);
      throw new Error('Failed to parse medical notes');
    }
  }

  async categorizeNotes(medicalText: string): Promise<CategoryExtraction> {
    try {
      const result = await generateObject({
        model: this.model,
...
Be thorough in your analysis and extract as much relevant information as possible.
      ...
      console.error('Error categorizing medical notes with Vercel AI:', error);
      throw new Error('Failed to categorize medical notes');
    }
  }
}

export const vercelAIParser = new VercelAIMedicalParser();
