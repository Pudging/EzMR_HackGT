...
export async function GET(
  ...
  try {
    ...
    console.log(
      `🔍 Looking for assessment data for patient MRN: ${patientId} in tenant: ${tenant.subdomain}`,
    );
    ...
    clinicalNotes.forEach((note: ClinicalNoteRecord) => {
      ...
      if (note.section?.length && note.section !== "OTHER") {
        ...
        if (
          bodyPartPatterns.includes(sectionBodyPart) &&
          !assessmentData[sectionBodyPart]
        ) {
