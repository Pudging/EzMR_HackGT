...
export async function GET(
  ...
  try {
    ...
    const transformedData = {
      ...
      insurance: {
        primary: patient.insurancePolicies.find((p: { isPrimary: boolean; providerName: string; policyNumber: string; groupNumber: string | null }) => p.isPrimary) ? {
          provider: patient.insurancePolicies.find((p: { isPrimary: boolean; providerName: string; policyNumber: string; groupNumber: string | null }) => p.isPrimary)!.providerName,
          policyNumber: patient.insurancePolicies.find((p: { isPrimary: boolean; providerName: string; policyNumber: string; groupNumber: string | null }) => p.isPrimary)!.policyNumber,
          groupNumber: patient.insurancePolicies.find((p: { isPrimary: boolean; providerName: string; policyNumber: string; groupNumber: string | null }) => p.isPrimary)!.groupNumber ?? undefined,
        } : undefined,
        secondary: patient.insurancePolicies.find((p: { isPrimary: boolean; providerName: string; policyNumber: string }) => !p.isPrimary) ? {
          provider: patient.insurancePolicies.find((p: { isPrimary: boolean; providerName: string; policyNumber: string }) => !p.isPrimary)!.providerName,
          policyNumber: patient.insurancePolicies.find((p: { isPrimary: boolean; providerName: string; policyNumber: string }) => !p.isPrimary)!.policyNumber,
        } : undefined,
      },
      
      emergencyContacts: patient.emergencyContacts.map((contact: { name: string; relation: string; phone: string }) => ({
        name: contact.name,
      ...
      allergies: patient.allergies.map((allergy: { substance: string; reaction: string | null; severity: string; notedOn: Date | null }) => ({
      ...
      medications: patient.medications.map((med: { name: string; dose: string | null; frequency: string | null; active: boolean }) => ({
        name: med.name,
      ...
      pastConditions: patient.pastMedical.map((event: { date: Date | null; type: string | null; description: string }) => ({
        date: event.date ? event.date.toISOString().split('T')[0] : "Unknown date",
        bodyPart: event.type ?? "OTHER",
        notes: event.description,
      })),
      
      immunizations: patient.immunizations.map((imm: { vaccine: string; administeredOn: Date; notes: string | null }) => ({
        vaccine: imm.vaccine,
        administeredOn: imm.administeredOn.toISOString().split('T')[0],
        notes: imm.notes ?? undefined,
      })),
      
      familyHistory: patient.familyHistory.map((history: { relation: string; condition: string; notes: string | null }) => ({
      ...
      vitals: (() => {
        ...
        const latestVitals: Record<string, { 
          type: string; 
          numericValue: number | null; 
          systolic: number | null; 
          diastolic: number | null; 
        ...
        patient.vitals.forEach((vital: { type: string; numericValue: number | null; systolic: number | null; diastolic: number | null; recordedAt: Date }) => {
      ...
      recentNotes: patient.notes
        .filter((note: { createdAt: Date; content: string }) => {
          ...
          const bodyParts = ['HEAD:', 'NECK:', 'CHEST:', 'HEART:', 'LEFT LUNG:', 'RIGHT LUNG:', 
